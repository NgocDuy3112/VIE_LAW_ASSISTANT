...
async def test_create_chat_completion_success():
    ...
    with patch("app.services.chat_completions.create_chat_completion_service", new=AsyncMock(return_value=expected_response)):
        async with ClientSession() as session:
            async with session.post(f"{BASE_URL}/llm-service/v1/chat/completions", json=[test_message.model_dump()]) as response:
                resp_json = await response.json()
    assert response.status == status.HTTP_200_OK
    assert resp_json["role"] == "assistant"
    # assert resp_json["content"][0]["content"] == "Hi!"


@pytest.mark.asyncio
async def test_create_chat_completion_empty_body():
    async with ClientSession() as session:
        async with session.post(f"{BASE_URL}/llm-service/v1/chat/completions", json=[]) as response:
            resp_json = await response.json()
...
async def test_create_chat_completion_timeout():
    test_message = Message(role="user", content=[{"role": "user", "content": "Hello"}])
    with patch("app.services.chat_completions.create_chat_completion_service", new=AsyncMock(side_effect=asyncio.TimeoutError)):
        async with ClientSession() as session:
            async with session.post(f"{BASE_URL}/llm-service/v1/chat/completions", json=[test_message.model_dump()]) as response:
                resp_json = await response.json()
...
async def test_create_chat_completion_rate_limit():
    ...
    with patch("app.api.chat_completions.limiter.limit", side_effect=HTTPException(status_code=429, detail="Rate limit exceeded. Please try again later.")):
        test_message = Message(role="user", content=[{"role": "user", "content": "Hello"}])
        async with ClientSession() as session:
            async with session.post(f"{BASE_URL}/llm-service/v1/chat/completions", json=[test_message.model_dump()]) as response:
